			...
			int height_out, int width_out, int channel_out, int stride)
{
	...
	if(i < hw_out && j < channel_out)
	{
		...
		if(threadIdx.y / width_out < RADIUS)
		{
			...
			// Load bottom padding
			tile[tileChn][tileRow + height_out][tileCol] = input[tileChn * height_in * width_in + (row + height_out) * width_in + col];
		}
		...
		if(threadIdx.y % width_out < RADIUS)
		{
			...
			// Load right padding
			tile[tileChn][tileRow][tileCol + width_out] = input[tileChn * height_in * width_in + row * width_in + col + width_out];
		}
		...
		if(threadIdx.y / width_out < RADIUS && threadIdx.y % width_out < RADIUS)
		{
			...
			tile[tileChn][tileRow - RADIUS][tileCol + width_out] = input[tileChn * height_in * width_in + (row - RADIUS) * width_in + col + width_out];
			
			// Load bottom-left corner padding
			tile[tileChn][tileRow + height_out][tileCol - RADIUS] = input[tileChn * height_in * width_in + (row + height_out) * width_in + col - RADIUS];
			
			// Load bottom-right corner padding
			tile[tileChn][tileRow + height_out][tileCol + width_out] = input[tileChn * height_in * width_in + (row + height_out) * width_in + col + width_out];
		}
