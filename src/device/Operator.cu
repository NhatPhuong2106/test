			...
			int height_out, int width_out, int channel_out, int stride)
{
	...
	if(i < hw_out && j < channel_out)
	{
		...
		if(threadIdx.y / width_out < RADIUS)
		{
			...
			// Load bottom padding
			tile[tileChn][tileRow + height_out + RADIUS][tileCol] = input[tileChn * height_in * width_in + (row + height_out) * width_in + col];
			//printf("if 2.2\n");
		}
		...
		if(threadIdx.y % width_out < RADIUS)
		{
			...
			// Load right padding
			tile[tileChn][tileRow][tileCol + width_out + RADIUS] = input[tileChn * height_in * width_in + row * width_in + col + width_out];
			//printf("if 3.2\n");
		}
		...
		if(threadIdx.y / width_out < RADIUS && threadIdx.y % width_out < RADIUS)
		{
			...
			tile[tileChn][tileRow - RADIUS][tileCol + width_out + RADIUS] = input[tileChn * height_in * width_in + (row - RADIUS) * width_in + col + width_out];
			//printf("if 4.2\n");
			// Load bottom-left corner padding
			tile[tileChn][tileRow + height_out + RADIUS][tileCol - RADIUS] = input[tileChn * height_in * width_in + (row + height_out) * width_in + col - RADIUS];
			//printf("if 4.3\n");
			// Load bottom-right corner padding
			tile[tileChn][tileRow + height_out + RADIUS][tileCol + width_out + RADIUS] = input[tileChn * height_in * width_in + (row + height_out) * width_in + col + width_out];
			//printf("if 4.4\n");
		}
